...
-(void)open: (CDVInvokedUrlCommand*)command
{
  ...
  if (dbname == NULL) {
    ...
    if (dbPointer != NULL) {
      ...
      NSLog(@"[SQLitePlugin]: Using db name: %@", dbname);
      sqlite3 *db;
      ...
      if (sqlite3_open(name, &db) != SQLITE_OK) {
        NSLog(@"Unable to open DB %@", dbname);
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"Unable to open DB"];
        return;
      } else {
        NSLog(@"Try to decode DB %@", dbname);
        // Extra for SQLCipher:
        ...
        if(sqlite3_exec(db, (const char*)"SELECT count(*) FROM sqlite_master;", NULL, NULL, NULL) == SQLITE_OK) {
          NSLog(@"[SQLitePlugin] database open!");
          dbPointer = [NSValue valueWithPointer:db];
          [openDBs setObject: dbPointer forKey: dbname];
          pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsString:@"Database opened"];
        } else {
          NSLog(@"[SQLitePlugin] Unable to encrypt DB!");
